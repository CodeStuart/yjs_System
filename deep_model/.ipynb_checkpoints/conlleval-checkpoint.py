...
Metrics = namedtuple('Metrics', 'tp fp fn xie acc prec rec fscore')
...
class EvalCounts(object):
    def __init__(self):
        ...
        #
        self.t_correct_xie = defaultdict(int)
    ...
    for line in iterable:
        ...
        if first_item != options.boundary:
            if correct == guessed and guessed_type == correct_type:
                ...
                counts.t_correct_xie[correct_type] += 1
...
def calculate_metrics(correct, guessed, total,xie):
    tp, fp, fn= correct, guessed-correct, total-correct
    xie1 = xie
    acc = 1.*xie1 / (xie1+fn+fp)
    p = 0 if tp + fp == 0 else 1.*tp / (tp + fp)
    r = 0 if tp + fn == 0 else 1.*tp / (tp + fn)
    f = 0 if p + r == 0 else 2 * p * r / (p + r)
    return Metrics(tp, fp, fn, xie, acc, p, r, f)


def metrics(counts):
    c = counts
    overall = calculate_metrics(
        c.correct_chunk, c.found_guessed, c.found_correct,c.found_correct
    )
    by_type = {}
    for t in uniq(list(c.t_found_correct) + list(c.t_found_guessed)):
        by_type[t] = calculate_metrics(
            c.t_correct_chunk[t], c.t_found_guessed[t], c.t_found_correct[t],c.t_correct_xie[t]
        )
...
def report(counts, out=None):
    ...
    for i, m in sorted(by_type.items()):
        out.write('%1s: ' % i)
        out.write('accuracy: %6.2f%%; ' % (100.*m.acc))
...
def report_notprint(counts, out=None):
    ...
    for i, m in sorted(by_type.items()):
        line = []
        line.append('%1s: ' % i)
        line.append('accuracy: %6.2f%%; ' % (100.*m.acc))
