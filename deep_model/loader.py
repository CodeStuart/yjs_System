...
from deep_model.bert import tokenization
...
from deep_model.data_utils import create_dico, create_mapping, zero_digits
from deep_model.data_utils import iob2, iob_iobes, get_seg_features
...
def load_sentences(path, lower, zeros):
    ...
    for line in codecs.open(path, 'r', 'utf8'):
        num+=1
		#zero_digits:将数字替换成字符'0' r.strip:删除尾部的空格
...
def char_mapping(sentences, lower):
    ...
    dico = create_dico(chars)#生词字典，统计词频
    dico["<PAD>"] = 10000001
    dico['<UNK>'] = 10000000

    char_to_id, id_to_char = create_mapping(dico)#按词频排序
...
def prepare_datasetb(sentences, max_seq_length, tag_to_id, lower=False, train=True):
    data = []
    for s in sentences:
        string = [w[0].strip() for w in s]
        char_line = ' '.join(string)  
...
def augment_with_pretrained(dictionary, ext_emb_path, chars):
    ...
    if chars is None:#词向量里没有的也加入字典，但是值为0
